...
uint8_t volumeInfo(uint8_t part) {
  ...
  if (part != 0) {
    if ((mbr->part[part-1].boot & 0X7F) !=0  ||
     ...
     fat_boot_t* fbs = (fat_boot_t*)Fat16::dbgCacheBlock(mbr->part[part-1].firstSector);
     if (fbs == 0) {
...
void loop() {
  ...
  Serial.println(d);
  cidDmp();
  uint32_t size = sd_card_size(&card);
